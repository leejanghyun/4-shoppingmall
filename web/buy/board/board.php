...
$result=mysql_query("SELECT count(*) FROM shoppingmall.$comment_tbl where name='$name'",$conn);
$result_row=mysql_fetch_row($result);
$total_row = $result_row[0];//현재 저장된 댓글의 총 갯수

if($total_row<=0) $total_row=0;
$total_page = floor(($total_row-1) /$page_size)+1;
$sql = "SELECT * FROM shoppingmall.$comment_tbl where name='$name'";
        ...
        <div id="boardView">
                ...
                <table class="boardtable">
                    ...
                    <tr>
                        ...
                        <td><span id="boardContent"><?php echo"<a href='".htmlspecialchars("../buy/board/commentModify.php?name=".$name."&no=".urlencode($row['no']))."'>".substr($row['title'],0,15)."</a>"?></span></td>
                        <td><span id="boardHit"><?php echo $row['view']?></span></td>

                    <?php  
                          if($row['id']===$_SESSION['id']){
                               echo "<td><span><a href='".htmlspecialchars("../buy/board/commentModify.php?name=".$name."&no=".urlencode($row['no']))."'>수정</a></span></td>";
                               echo "<td><span><a href='".htmlspecialchars("../buy/board/server/comment_delete.php?name=".$name."&no=".urlencode($row['no']))."'>삭제</a></span></td>";?>
                        </tr><br>
                    <?php     }
                        }
                    }?>
                    ...
                    <?php
                        ...
                        if(!(1>( ($current_page-1)/8))){
                          echo "<a class='L_no' href='".htmlspecialchars("./main.php?id=board&page=".urlencode( $current_page-7))."'> < </a>";
                        }
